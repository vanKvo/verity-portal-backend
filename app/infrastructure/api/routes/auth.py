...
from app.domain.models.user import User as DomainUser, InvalidDomainError
...
from app.infrastructure.api.schemas.auth import UserCreate, Token
...
def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        domain_user = DomainUser.create(email=user_data.email, raw_password=user_data.password)
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    # Create domain user temporarily just to verify password
    domain_user = DomainUser(email=db_user.email, hashed_password=db_user.hashed_password, role=db_user.role)
