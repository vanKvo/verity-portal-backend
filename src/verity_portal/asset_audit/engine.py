...
class AssetReconciliationEngine:
    ...
    def run_audit(self):
        ...
        if ghost_assets > 0:
            self._notify_it_department(ghost_assets)
            
        if wasted_spend > 0:
            self._notify_finance_department(wasted_spend)
            
        logger.info(f"Audit completed. Found {ghost_assets} Ghost Assets and {wasted_spend} Wasted Spend anomalies.")
        return {"ghost_assets": ghost_assets, "wasted_spend": wasted_spend}
